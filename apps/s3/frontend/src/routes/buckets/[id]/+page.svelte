<script lang="ts">
    ...
    import { Card, Badge, Button, Input, Separator, DropdownMenu, Tabs } from 'positron-components/components/ui';
...
{#if selectedBucket}
    <div class="h-full flex flex-col">
        ...
        <div class="flex-1 p-6">
            <Tabs.Root value="files" class="h-full flex flex-col">
                ...
                <!-- Files Tab -->
                <Tabs.Content value="files" class="flex-1 mt-6 space-y-4">
                    <!-- File Manager Toolbar -->
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <!-- Breadcrumb -->
                            <div class="flex items-center gap-2">
                                <Button variant="ghost" size="sm" onclick={() => currentPath = []}>
                                ...
                                {#each currentPath as pathSegment, index}
                                    ...
                                    <Button 
                                        variant="ghost" 
                                        size="sm"
                                        onclick={() => currentPath = currentPath.slice(0, index + 1)}
                                    >
                                        {pathSegment}
                                    </Button>
                                {/each}
                        ...
                        <Card.Content class="p-6">
                            {#if filteredFiles.length === 0}
                                <div class="text-center py-12">
                                    ...
                                    <p class="text-muted-foreground mb-4">
                                        {searchTerm ? 'Keine Dateien entsprechen Ihrer Suche.' : 'Dieses Verzeichnis ist leer.'}
                                    </p>
                                    <Button onclick={handleUpload}>
                                        <Upload class="h-4 w-4 mr-2" />
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                                    {#each filteredFiles as file}
                                        ...
                                        <div class="group relative">
                                            <Card.Root class="hover:shadow-md transition-all duration-200 cursor-pointer border-2 {selectedFiles.has(file.id) ? 'border-primary' : 'border-border'}">
                                            ...
                                            <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <DropdownMenu.Root>
                                                    ...
                                                    <DropdownMenu.Content>
                                                        ...
                                                        <DropdownMenu.Item onclick={() => handleDelete(file)} class="text-destructive">
                                            ...
                                            <button
                                                ...
                                                aria-label="{file.name} {file.type === 'folder' ? 'Ordner öffnen' : 'Datei auswählen'}"
                            ...
                            {:else}
                                ...
                                <div class="space-y-1">
                                    ...
                                    {#each filteredFiles as file}
                                        {@const IconComponent = getFileIcon(file)}
                                        <div class="grid grid-cols-12 gap-4 px-4 py-3 hover:bg-muted/50 rounded-lg cursor-pointer group {selectedFiles.has(file.id) ? 'bg-primary/10' : ''}">
                                            <button 
                                                class="col-span-6 flex items-center gap-3 text-left w-full bg-transparent border-0 p-0" 
                                                onclick={() => handleFileClick(file)}
                                            >
                                                <IconComponent class="h-5 w-5 text-muted-foreground flex-shrink-0" />
                                            ...
                                            <div class="col-span-3 flex items-center text-sm text-muted-foreground">
                ...
                <Tabs.Content value="settings" class="mt-6">
                    <Card.Root>
                        <Card.Header>
                            ...
                            <Card.Description>Verwalten Sie die Konfiguration Ihres Buckets</Card.Description>
                        </Card.Header>
                        <Card.Content class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                                                            <div class="text-sm font-medium text-muted-foreground">Bucket-Name</div>
                                    <div class="p-3 bg-muted rounded-lg">
                                        <code class="text-sm">{selectedBucket.name}</code>
                                    </div>
                                </div>
                                <div class="space-y-2">
                                                                            <div class="text-sm font-medium text-muted-foreground">Region</div>
                                    <div class="p-3 bg-muted rounded-lg">
                            ...
                            <div class="space-y-2">
                                                                        <div class="text-sm font-medium text-muted-foreground">Beschreibung</div>
                                <p class="text-sm text-muted-foreground">{selectedBucket.description}</p>
                            </div>
                        </Card.Content>
                    </Card.Root>
                </Tabs.Content>
                ...
                <Tabs.Content value="permissions" class="mt-6">
                    <Card.Root>
                        <Card.Header>
                            ...
                            <Card.Description>Verwalten Sie die Berechtigungen für dieses Bucket</Card.Description>
                        </Card.Header>
                        <Card.Content>
                            <div class="space-y-4">
        ...
        <div class="text-center space-y-4 p-8">
            ...
            <div class="space-y-2">
                ...
                <p class="text-muted-foreground max-w-md">
                    Klicken Sie auf ein Bucket in der Seitenleiste, um detaillierte Informationen und das Dateisystem anzuzeigen
                </p>
            </div>
